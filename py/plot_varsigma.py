        ...
        for im in range(nmulti):
          ...
          #_im = im if im < 2 else im + 1
            ...
            if plotmodel == 'dispersion' or plotmodel == 'both': 
              ...
              ax.plot(rm[nnan],dm_disp[nnan] ,'-',color=col[1],label=lgt,linewidth=lwidth)

            if plotmodel == 'GSM' or plotmodel == 'both':
              nnan = np.isfinite(dm_gsm)
              ax.plot(rm[nnan],dm_gsm[nnan],'-',color=col[1],label=lgt,linewidth=lwidth)
...
#pl.show()
pl.savefig(plotmodel + 'varsig.pdf',bbox_inches='tight')
