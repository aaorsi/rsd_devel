...
for igt in range(ngtype):
  gt = Galtype[igt]
  col = 'blue' if gt == 'Starforming' else 'red'
            ...
            for _q0 in q0:
              for _g0 in g0:
                for s2 in sfog:
                  ...
                  if plottype == 'sigma':
                    ax.plot(data[:,0],data[:,1],color=col,linewidth=2,label=gt)
                    if igt == 0:
                    ...
                    ax.plot(data[:,0],data[:,2],color=col,linewidth=2,label=gt)
                    ax.set_xlabel(r'$\log(M_{\rm halo})$',fontsize=15)
                    ax.fill_between(data[:,0],data[:,3],data[:,4],color=col,alpha=0.25)
                    ax.set_title('n = '+nd,fontsize=15)
                    if idens == 0:
                      ax.legend(loc='upper left')
